...
def test_b933_sqlite_and_jsonl_still_agree_after_the_cap_change(tmp_path):
    ...
    the 2,000 per-source cap — must extract the IDENTICAL, identically-truncated 2,000
    defs through both readers, proving the two-tier logic wasn't ported divergently."""
    from clawseccheck import trajectorystore as ts
...
def test_b933_single_source_exceeding_its_own_cap_is_a_documented_residual(tmp_path):
    ...
    one SQLite database, landing after that source's own per-source cap is exhausted --
    the check must disclose the truncation (never silently drop it) and must NOT report
    a false FAIL it can no longer see, so PASS-with-truncated is the honest verdict."""
    ...
    rows = [(f"s{i}", i, ev) for i, ev in enumerate(benign_events)]
    rows.append(("s_poison", len(rows), poisoned_event))
