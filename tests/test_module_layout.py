...
_EXEMPT = {
    ...
    "risk.py": "~2,937 lines (restated 2026-09-17 — was ~2,421) — the combinational "
               ...
               "cycle.",
    "skillast.py": "~12,319 lines. Restated 2026-09-24, CLAWSECCHECK-B-906: +523 over the "
                   "prior ~11,796 restatement, crossing the 500-line tolerance band. Added "
                   "`_RefResolver` — positive-only import-provenance resolution for TT5's "
                   "os.getenv/os.environ vocabulary (an aliased import, a plain local alias, "
                   "or a foldable indirect access through getattr/__dict__/vars/sys.modules/"
                   "__import__/importlib.import_module resolves to a canonical dotted name; "
                   "reuses shippedexec._FileFacts.dotted()/sole()/_legb_lookup() rather than "
                   "re-deriving reaching definitions), its env vocabulary and mutated-path "
                   "guard, and threading `ref_res` through the seven existing TT5 call sites "
                   "as an additional `or` disjunct (never a replacement) so the change is "
                   "provably monotone — see tests/test_b906_ref_resolver.py's test_monotone_* "
                   "guards. Replaces round 4's Part B/C suppressor attempt (PF1/PF2, a "
                   "must-NOT-alias proof used to SUPPRESS a base conviction), which was built "
                   "and abandoned on a separate branch without ever landing here — this "
                   "restatement is B-906's only line-count effect on this file. Same shape as "
                   "every prior restatement below: no new parser family, still python/shell/js; "
                   "the file's own split remains deferred to the same later cycle (I-022 "
                   "secondary target) the B-850 engine extraction candidate already named. "
                   "Prior text retained verbatim beneath this note for its own history: "
                   "restated 2026-09-23, merging three concurrent growth lines from a common ~7,267-line B-643 baseline (2026-09-16): B-830 round 2, "
