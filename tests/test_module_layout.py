...
_EXEMPT = {
    ...
    "checks/_vet.py": "~9,185 lines (restated 2026-09-24, B-879 round 5 — was ~9,165; "
    ...
    "risk.py": "~2,937 lines (restated 2026-09-17 — was ~2,421) — the combinational "
               ...
               "cycle.",
    "skillast.py": "~16,984 lines (further restated 2026-09-25 on top of "
                   "the 2026-09-24d restatement immediately below, whose own ~16,213 claim "
                   "was already 734 lines stale by the time this restatement was written -- "
                   "this file's own growth is not fully reconstructed line-by-line back to "
                   "that claim; only this restatement's OWN delta is known precisely: the "
                   "round-5 loop credential-exfiltration fix (+37 net over the 16,947 this "
                   "branch started from) closed an operator-reference exemption bypass in "
                   "the loop DIRECT role -- the splice only ever recognized a bare $var/"
                   "${var} reference, leaving a parameter-expansion operator's own suffix as "
                   "dangling literal text once substituted, which still read as the clean "
                   "certified token to the credential-path regex's own substring-search "
                   "content check. Fix: a real bare-vs-operator classifier "
                   "(shellwords.param_refs, in shellwords.py, not counted here) replaces the "
                   "splice's use of the loose legacy regex, an operator-reference-span "
                   "refusal was added to the shared exemption engine (fail-closed-convict "
                   "outside every TLS-material value span), and a TLS-material flag's own "
                   "position-only excusal was narrowed to exclude a live command "
                   "substitution hiding inside that same value. The net is small because "
                   "three now-dead enumeration-based helper functions (~180 lines) were "
                   "retired in the same change, offsetting the new mechanism and its "
                   "in-source history (this is round 5 of the same bypass class; each prior "
                   "round's own repro and fix stays in place as a regression record, the "
                   "same convention the rest of this file already follows). Same "
                   "TT5-adjacent shell-credential-taint family every entry below already "
                   "touches -- not a new parser family. "
