...
_EXEMPT = {
    ...
    "monitor.py": "~4,267 lines — the drift/monitor comparison layer, and for the first "
                  "time this entry records a SHRINK. C-433's first slice moved the local "
                  "STORE out to monitorstore.py: 32 top-level names, 720 lines, chosen "
                  "because it references ZERO WATCHED_DIMENSIONS keys and therefore cannot "
                  "perturb the AST derivation in test_c417_snapshot_enablers.py. Snapshot "
                  "output proven byte-identical on home_safe and home_vuln across the move. "
                  "This does NOT close C-433 — the file is still 3.5x the budget, and the "
                  "remaining bulk is diff_with_notes at ~1,635 lines whose 202 locals "
                  "include 61 read by more than one arm, which is a data-flow rewrite "
                  "rather than a file move. Previously: "
                  "time on 2026-08-26, and the reason matters more than the number: E-076's "
                  "successors added five dimensions to this one file in a single day "
                  "(plugins, the resolved-config digest, named dimensions, the native_count "
                  "guard, and F-179's host-persistence arm), taking it 4,299 -> 4,847 in "
                  "hours. This guard exists to make that visible rather than to be silenced, "
                  "so read the growth as the signal: C-433 (split monitor.py per dimension) "
                  "is filed, measured, and is now the largest piece of structural debt in "
                  "this module — diff_with_notes alone is ~1,635 lines with 61 locals read "
                  "by more than one arm. Do not restate this a third time without landing "
                  "C-433 or a slice of it. Previously: "
                  "~3,798 on 2026-08-25: it had already drifted to 4,247 "
                  "unnoticed (+12%, one point under this guard's tripwire) before this "
                  "restatement, and this change's own chain_provenance_note (B-582 — "
                  "wiring the tamper-evident chain check into the --trend/--watch-log "
                  "VIEWERS, not just the standalone --verify-history/--verify-events "
                  "commands) added ~50 more, to 4,299, which is what tripped it. The "
                  "~450 lines of unattributed prior drift are NOT this change's — most "
                  "of it is a run of fail-open corrections to the skill-provenance/"
                  "supply-chain drift dimensions (e.g. 'never report a clean verdict "
                  "over ground that was not read', 'compare every install record with "
                  "itself, and stop electing a winner', 'stand every skill-provenance "
                  "arm down on an undeterminable record', 'let drift detection see a "
                  "change made in an $include fragment'): each landed here because "
                  "snapshot()/diff() is where a dimension's comparison lives, the same "
                  "reason report.py's own entry below names for honesty fixes landing "
                  "there. The drift-snapshot/diff engine (snapshot() builds every "
