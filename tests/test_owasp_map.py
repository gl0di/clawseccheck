...
**B-522 — this file was deleted by accident and is restored here. Do not remove it with
the next OWASP-2026 attempt.** It has existed since v1.6.0 and guards the 2025 map alone.
What happened: `b01e46b` swept another session's in-flight 2026 work into an unrelated
commit (the `git add -A` hazard B-498 tracks), which added 80 lines of 2026 tests to this
file. The revert `a630f84` then deleted all 243 lines rather than returning it to its
pre-sweep 163 — so a clean-up of work that had never been reviewed also took 16 tests
that had guarded `OWASP_MAP` for months, and nothing noticed because deleting a test file
cannot fail a suite.

This is the pre-sweep version restored verbatim. The 2026 migration is C-390's, still
open; when it lands it ADDS to this file.
"""
import json
