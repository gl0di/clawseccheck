...
all (so they can't even surface as a finding). Updated 2026-07-25 for v3.57.0.

Current catalog: A1 plus the B-series, C-series, and T-series (behavioral) — 157 catalogued,
...
| MCP server trust | B15, B24, B47, B333, `--vet-mcp` | Unpinned installs, plaintext transport, env/secret passthrough, broad scopes; B333 (F-143/W2.1, grounded against dist openclaw@2026.7.1-2) flags a config-embedded MCP tool declaring a `readOnlyHint`/`destructiveHint`/`openWorldHint`/`idempotentHint` annotation — OpenClaw's registration path stores only `{serverName, safeServerName, toolName, title, description, inputSchema, fallbackDescription}` and never reads these SDK-vendor-typed hints, so any operator policy relying on them is silently unenforced. WARN-only (a host limitation, not server wrongdoing — never worded as the server lying); UNKNOWN when the only annotation evidence available comes from OpenClaw's own retained/compiled form (trajectory records, an `openclaw mcp probe --json` dump), which structurally never carries annotations at all and so proves nothing about what was originally declared `[CHECK: B15, B24, B47, B333]` |
...
| Plugin holding the runtime tool-result-rewrite contract | B187 | A plugin declaring `contributions.contracts.agentToolResultMiddleware` can rewrite every tool result before it reaches the model — a runtime interception point stronger than a single poisoned MCP server. WARN-only capability disclosure, gated on `origin != "bundled"`: 67 of 69 plugins on a stock install are bundled and already carry this contract, so an ungated check would fire on every clean machine. Reads the `plugins_json` column of `installed_plugin_index` via its own independent `SELECT`, deliberately separate from B177's `install_records_json` read, so a schema variant missing one column degrades only that column's checks to UNKNOWN rather than both. Never asserts malice — only that the contract is present, on which non-bundled plugin, and whether it is enabled `[CHECK: B187]` |
