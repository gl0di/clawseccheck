...
| Plugin / MCP install-time trust | B5, B15, B24, B42, B57, B166, C047 | B57 flags `permissionMode=approve-all`; C047 is a manual-review flag for a non-local MCP endpoint; B166 (C-211, grounded against the real OASB registry corpus — 0/2988 benign FP, 1/166 malicious caught) flags a known paste/exfiltration host (webhook.site, ngrok, pastebin, `.onion`, …) named in an MCP server's own `command`/`args` — advisory WARN, never scored `[CHECK: B5, B15, B24, B42, B57, B166, C047]` |
