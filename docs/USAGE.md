...
- **`--full`** runs the audit and then appends: self-test scenario generation, the MCP vet,
  ...
  **adjudication phase** (the same borderline-band judge packet `--judge-packet` produces,
  ...
  - **`--judged-bundle PATH`** (only with `--full`, `-` for stdin) feeds back one file holding
    ...
    `docs/OUTPUT_SCHEMA.md` §12 for the exact shape). Every entry (`canary`/`redteam`/
    `dryrun`/`multiturn`) is also cross-checked against your own local trajectory when
    one is readable (F-193/F-194): a `canary` id that could not have come from the
    bucket's own `seed`, or any of the four's verdict the trajectory disproves, is
    dropped before it can complete the live-behaviour ledger layer or set the cap — the
    same treatment as no submission at all, not a special case. `--full`'s own printed section is
